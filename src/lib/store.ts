...
export function useStore() {
  ...
  const updateFournisseurs = useCallback((fn: (prev: Fournisseur[]) => Fournisseur[]) => {
    setFournisseurs(prev => {
      ...
      if (userId) {
        const { added, removed, updated } = diffArrays(prev, next);
        if (added.length) supabase.from('fournisseurs').insert(added.map(f => fournisseurToDb(f, userId))).then();
        if (updated.length) {
          updated.forEach(f => supabase.from('fournisseurs').update(fournisseurToDb(f, userId)).eq('id', f.id).then());
        }
  ...
  const updateDevis = useCallback((fn: (prev: Devis[]) => Devis[]) => {
    setDevis(prev => {
      ...
      if (userId) {
        const { added, removed, updated } = diffArrays(prev, next);
        if (added.length) supabase.from('devis').insert(added.map(d => devisToDb(d, userId))).then();
        if (updated.length) {
          updated.forEach(d => supabase.from('devis').update(devisToDb(d, userId)).eq('id', d.id).then());
        }
