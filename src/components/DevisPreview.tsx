            ...
            <table className="w-full mb-6 text-xs border-collapse">
              ...
              <thead>
                {/* Centrage vertical : texte directement dans <th>, align-middle Tailwind.
                    Pas de <div> interne (perturbe html2canvas sur les table-cells).
                    padding symétrique haut/bas pour l'espace visuel, vertical-align:middle
                    pour le centrage réel via la sémantique table-cell. */}
                {/* ── Ligne 1 : DÉSIGNATION + groupes ── */}
                <tr className="bg-[#CC0000] text-white">
                  ...
                  <th colSpan={2} className="border-l border-white/20 align-middle"
                      style={{ padding: '4px 4px', fontSize: '12px', fontWeight: 'bold', textAlign: 'center', whiteSpace: 'nowrap' }}>Conso. Est.</th>
                  <th colSpan={3} className="border-l border-white/20 align-middle"
                      style={{ padding: '4px 4px', fontSize: '12px', fontWeight: 'bold', textAlign: 'center' }}>Condit.</th>
                  <th colSpan={3} className="border-l border-white/20 align-middle"
                      style={{ padding: '4px 4px', fontSize: '12px', fontWeight: 'bold', textAlign: 'center' }}>Prix</th>
                </tr>
                ...
                <tr className="bg-[#CC0000] text-white">
                  ...
                  <th className="border-l border-white/20 align-middle"
                      style={{ padding: '2px 4px', fontSize: '11px', textAlign: 'right', whiteSpace: 'nowrap' }}>kg/m²</th>
                  <th className="align-middle"
                      style={{ padding: '2px 4px', fontSize: '11px', textAlign: 'center', whiteSpace: 'nowrap' }}>KG</th>
                  <th className="border-l border-white/20 align-middle"
                      style={{ padding: '2px 4px', fontSize: '11px', textAlign: 'right', whiteSpace: 'nowrap' }}>kg</th>
                  <th className="align-middle"
                      style={{ padding: '2px 4px', fontSize: '11px', textAlign: 'right', whiteSpace: 'nowrap' }}>Unité</th>
                  <th className="align-middle"
                      style={{ padding: '2px 4px', fontSize: '11px', textAlign: 'center', whiteSpace: 'nowrap' }}>KG</th>
                  <th className="border-l border-white/20 align-middle"
                      style={{ padding: '2px 4px', fontSize: '11px', textAlign: 'right', whiteSpace: 'nowrap' }}>Unité</th>
              ...
              <tbody>
                ...
                <tr className="bg-muted/50 border-b-2 border-[#CC0000] text-xs italic font-medium">
                  <td />
                  <td className="py-1.5 px-1 text-right">{sumConsoKgM2 > 0 ? fmt(sumConsoKgM2) : ''}</td>
                  <td className="py-1.5 px-1 text-right">{sumTotalKg > 0 ? fmt(sumTotalKg, 1) : ''}</td>
                  <td /><td />
                  <td className="py-1.5 px-1 text-right">{sumCondKg > 0 ? fmt(sumCondKg, 1) : ''}</td>
                  <td colSpan={3} />
                </tr>
                )}
                ...
                {(() => {
                  ...
                  <Fragment key={l.id}>
                    {showHeader && (
                      <tr className="bg-primary/[0.03] border-t border-[#CC0000]" data-pdf-no-break-after="true">
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                          ...
                          {l.variantesChoisies && (() => {
                            ...
                            return [...new Set(Object.values(l.variantesChoisies))].sort((a, b) => {
                              ...
                              if (rs) return (
                                <span key={i} style={{ backgroundColor: rs.backgroundColor, color: rs.color, padding: '0 8px', borderRadius: '4px', fontSize: '0.7rem', fontWeight: 'bold', letterSpacing: '0.04em', whiteSpace: 'nowrap', flexShrink: 0, height: '26px', lineHeight: '26px', display: 'inline-block', ...(rs.border ? { border: rs.border } : {}) }}>{rs.quartz ? `${rs.quartz} · RAL ${rs.ralNum}` : `RAL ${rs.ralNum}`}</span>
                              );
                              const imgUrl = prod?.variantes?.flatMap(d => d.options).find(o => o.label === label)?.imageUrl;
                              if (imgUrl) {
                        ...
                        <td className={`py-2 ${myGrpS ? 'pl-4' : ''}`}>
                          ...
                          {l.variantesChoisies && (() => {
                            ...
                            return [...new Set(Object.values(l.variantesChoisies))].sort((a, b) => {
                              ...
                              const rs = getRalStyle(label);
                              if (rs) return (
                                <span key={i} style={{ backgroundColor: rs.backgroundColor, color: rs.color, padding: '0 8px', borderRadius: '4px', fontSize: '0.7rem', fontWeight: 'bold', marginLeft: '6px', display: 'inline-block', verticalAlign: 'middle', letterSpacing: '0.04em', height: '26px', lineHeight: '26px', ...(rs.border ? { border: rs.border } : {}) }}>{rs.quartz ? `${rs.quartz} · RAL ${rs.ralNum}` : `RAL ${rs.ralNum}`}</span>
                              );
                              const imgUrl = prod?.variantes?.flatMap(d => d.options).find(o => o.label === label)?.imageUrl;
                              if (imgUrl) {
