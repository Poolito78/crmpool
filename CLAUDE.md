...
`planDirectionnel` (carnet de plans Kadri lu sans IA, chiffré par ensemble → DF50/DR50 Lapérouse P50 par défaut, Vasco = option F, Urville hors grille, grands formats en Tasman PAL = variante Odoo D3.l.h.Cx.ST.IS.BRUT (face standard) ; ni pose ni dépose) ·
