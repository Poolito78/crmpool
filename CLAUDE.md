...
⚠️ **`odoo-prix` : la LISTE DE PRIX du client fait foi.** Une grille R1-R4 ne tarife que dans deux cas — un contrat cadre réellement rattaché (objet Studio « CCI10019 TARIF R4 … », cf. `contrat-cadre-REFLEX.md`), ou un niveau **imposé** au sélecteur. Il y avait un repli automatique sur la grille du niveau affiché faute de contrat rattaché ; il est retiré. Mesuré sur le devis Odoo **AF036911** (MGD, liste de prix « 30/70/72/… ») : la grille R4 cotait AK3.700.C1.BTR.R.IS.BRUT à 39,41 € quand le devis émis le facture **37,475 €**. Le repli était invisible — la ligne avait bien un article et bien un prix. `niveauDefaut` reste envoyé (il tarife les lignes chiffrées à la grille faute d'article Odoo, `prixPanneau`) mais ne décide plus rien côté serveur.
