...
- **`modelesIA.ts`** — Les modèles Gemini appelés depuis le navigateur, en UN SEUL endroit. ⚠️ **UN MODÈLE RETIRÉ CASSE TOUT EN SILENCE** : le 11/09/2026 Google a arrêté `gemini-2.0-flash` et `gemini-2.0-flash-lite` (« Shut down »), les six appels de l'application répondaient **404**, et rien ne le disait à l'écran — la lecture des signatures rendait `null` comme si l'image n'avait rien donné. Le nom était écrit en dur dans cinq fichiers. `MODELES_GEMINI` est une **chaîne de repli** (on garde le premier qui répond), `urlGemini(modele, cle)` construit l'appel. Le prochain retrait se corrige ICI. ⚠️ `VITE_GEMINI_API_KEY` est une variable de BUILD : elle est incrustée dans le bundle et lisible par quiconque ouvre l'application — à restreindre par référent chez Google, et à déplacer un jour dans une Edge Function. Reste hors du module : `google/gemini-2.0-flash-exp:free` dans `VeilleConcurrence.tsx`, qui passe par OpenRouter, un autre catalogue.
