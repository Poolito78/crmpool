...
- **`modelesIA.ts`** — Les appels Gemini du navigateur… qui n'en partent plus. ⚠️ **LA CLÉ ÉTAIT PUBLIQUE** : `VITE_GEMINI_API_KEY` est une variable de BUILD, Vite l'inscrivait en clair dans le bundle et elle s'affichait jusque dans les URL de la console. Tout passe désormais par l'Edge Function **`gemini`**, où la clé est le secret `GEMINI_API_KEY` — le même qu'`analyze-email`. `appelerGemini(corps, modeles?)` envoie le corps Gemini tel quel et rend la réponse brute ; `texteGemini(data)` en extrait le texte. ⚠️ **`verify_jwt = true`** sur la fonction : un relais ouvert offrirait à tout venant un accès Gemini facturé au propriétaire de la clé, soit exactement ce que la clé publique permettait. ⚠️ **PLUS DE GARDE `if (geminiKey)`** dans les chaînes de repli : le navigateur ne PEUT plus savoir si le secret est posé, et garder une variable de build pour faire semblant remettrait la clé dans le bundle. On essaie, et l'échec parle. ⚠️ **UN MODÈLE RETIRÉ CASSE TOUT EN SILENCE** : le 11/09/2026 Google a arrêté `gemini-2.0-flash` et `-lite`, les six appels répondaient **404**, et rien ne le disait à l'écran — la lecture des signatures rendait `null` comme si l'image n'avait rien donné. La chaîne de repli vit maintenant dans la fonction (elle fait foi, même pour un front en cache) et la réponse d'erreur porte le détail de CHAQUE tentative : c'est ce qui distingue un modèle retiré (404) d'une clé refusée (403), d'un quota (429) ou d'une API non activée sur le projet Google. Reste hors du module `google/gemini-2.0-flash-exp:free` dans `VeilleConcurrence.tsx`, qui passe par OpenRouter, un autre catalogue.
...
- **`bridesDevis.ts` / `railsPanneaux.donnees.ts`** — **Une bride par rail**, le nombre de rails se LIT dans la table du catalogue. On n'invente jamais : famille ou cote absente → « à vérifier », hors du total. La signalisation temporaire a ses rails elle aussi (segment `.R.` sur AK/BK/KC/KD/CK) : triangles et disques lisent la table de la police, les rectangles valent 2 rails quelle que soit la taille, et un article créé pour l'affaire (sans code) n'est compté que s'il annonce « KIT RAIL » et que sa cote tient sous 850. ⚠️ La CLASSE (`C1`, `C2`, `C3`) se faisait prendre pour un code de panneau dans `AK3.700.C1.BTR…` — `CLASSE_SEULE` l'écarte. Le détail complet est dans les deux en-têtes de source (`claude/brides-et-rails.md` n'existe pas, malgré le renvoi de `reprise-2026-09.md`). `fixationsPour` chiffre ces brides pour un ensemble et `fixationDeSection` dit lequel des trois articles la section appelle — collier galvanisé sur Ø60, bride acier sur les profils carrés, collier alu sur les tubes : choisir la section sans changer la fixation facturerait des colliers Ø60 sur du 80×80.
- **`tarifPanneaux.ts` / `tarifPanneaux.donnees.ts`** — la grille R0–R4 des panneaux de police, panonceaux et supports. `supportPour` déduit la longueur du support (ancrage + hauteur libre + porté, arrondi au standard supérieur) et la tarife dans la section demandée ; les tables de `SUP_SECT` étant trouées, une longueur absente se prolonge au mètre linéaire et `prixExact` le dit. ⚠️ `supportPour` **ne compte plus les fixations** — c'était « une par élément porté », ce qui ne correspond à rien face à la règle « une bride par rail ». `estPoseBasse` isole les B21 et J5, dont la hauteur sous panneau vaut par défaut la hauteur du panneau et non 2,10 m.
